<?php
/*
Plugin Name: WooCommerce Gateway Yeahpay
Description: Custom payment gateway for Yeahpay.
Version: 1.0
Author: The Web Founders
*/

add_action('plugins_loaded', 'init_paycanada_gateway');

function init_paycanada_gateway() {
    if (!class_exists('WC_Payment_Gateway')) return;

    class WC_PayCanada_Gateway extends WC_Payment_Gateway {
        public function __construct() {
            $this->id = 'paycanada';
            $this->icon = ''; // Add an icon URL if needed.
            $this->has_fields = true;
            $this->method_title = 'PayCanada';
            $this->method_description = 'Pay with PayCanada';
            ...
            add_action('woocommerce_credit_card_form_start', array($this, 'paycanada_credit_card_form_start'));
            add_action('woocommerce_credit_card_form_end', array($this, 'paycanada_credit_card_form_end'));
        }
        ...
        public function init_form_fields() {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => 'Enable PayCanada Gateway',
                    'default' => 'yes',
                ),
                'title' => array(
                    'title' => 'Title',
        ...
        public function paycanada_credit_card_form_start() {
            echo '<div id="paycanada-card-details">';
        }

        public function paycanada_credit_card_form_end() {
            echo '</div>';
        }
    ...
    function add_paycanada_gateway($methods) {
        $methods[] = 'WC_PayCanada_Gateway';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_paycanada_gateway');
}
